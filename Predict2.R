...
d <- data.frame(FirstWord = w1, SecondWord = w2, ThirdWord = w3, Occurrences = x[ , 1 ])
write.csv(d, "caprojshinyapp/bigram.csv")
# Find the most frequent sequences for each 3gram and put them into a data.frame
#
#df <- aggregate(Occurrences ~ FirstWord + SecondWord, data = d, FUN = max)
