...
w3 <- sapply(y, FUN = function(a) a[ 3 ]) # w3 - the third word vector
w4 <- sapply(y, FUN = function(a) a[ 4 ]) # w4 - the fourth word vector

d <- data.frame(FirstWord = w1, SecondWord = w2, ThirdWord = w3, FourthWord = w4, Occurrences = x[ , 1 ])
write.csv(d, "caprojshinyapp/trigram.csv")
# Find the most frequent sequences for each 3gram and put them into a data.frame
#
#df <- aggregate(Occurrences ~ FirstWord + SecondWord + ThirdWord, data = d, FUN = max)
#df <- df[ df$Occurrences > 2, ] # take relatively frequent ngrams 
#u <- as.vector(df$FirstWord)
#factor(u)
